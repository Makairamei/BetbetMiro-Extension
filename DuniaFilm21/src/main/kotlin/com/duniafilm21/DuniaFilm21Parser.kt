...
internal object DuniaFilm21Parser {
    ...
    private fun parseCard(api: MainAPI, element: Element): SearchResponse? {
        ...
        val url = DuniaFilm21Utils.absoluteUrl(api.mainUrl, anchor.attr("href")) ?: return null
        if (!DuniaFilm21Utils.isContentUrl(url)) return null
        ...
        val title = DuniaFilm21Utils.cleanTitle(rawTitle)
        if (title.isBlank() || title.equals("Watch", true) || title.equals("Watch Movie", true) || title.length < 2) return null

        val text = element.text()
        val poster = DuniaFilm21Utils.pickImage(api.mainUrl, image, element)
        val type = DuniaFilm21Utils.typeFrom(url, title, text)
        val quality = element.selectFirst(".quality, .Qlty, .mli-quality, .calidad, span.quality, span:contains(HD), span:contains(CAM)")?.text()
        val year = DuniaFilm21Utils.extractYear(text) ?: DuniaFilm21Utils.extractYear(title)

        return api.newMovieSearchResponse(title, url, type) {
            posterUrl = poster
            quality?.let(DuniaFilm21Utils::cleanText)?.takeIf { it.isNotBlank() }?.let { addQuality(it) }
            this.year = year
        }
    }

    suspend fun parseLoad(api: MainAPI, url: String, doc: Document): LoadResponse? {
        val title = DuniaFilm21Utils.cleanTitle(
        ...
        val poster = DuniaFilm21Utils.extractMetaImage(api.mainUrl, doc)
        val plot = DuniaFilm21Utils.cleanText(
        ...
        val tags = doc.select("a[rel=tag], .sgeneros a, .genxed a, .mta a, a[href*='/genre/'], a[href*='/country/']")
            .map { DuniaFilm21Utils.cleanText(it.text()) }
        ...
        val year = DuniaFilm21Utils.extractYear(metadataText) ?: DuniaFilm21Utils.extractYear(title)
        val duration = DuniaFilm21Utils.extractDuration(metadataText)
        val rating = DuniaFilm21Utils.extractRating(doc.selectFirst(".rating, .imdb, .dt_rating_vgs, .starstruck-rating")?.text())
            ?: DuniaFilm21Utils.extractRating(metadataText)
        val episodes = parseEpisodes(api, doc)
        val type = DuniaFilm21Utils.typeFrom(url, title, metadataText)
    ...
    private fun parseEpisodes(api: MainAPI, doc: Document): List<Episode> {
        return doc.select("a[href*='/episode/'], a[href*='/eps/'], a[href*='?episode='], .episodios a[href], .episode a[href], .se-c a[href], .eplister a[href], .all-episodes a[href]")
            .mapNotNull { anchor ->
                val epUrl = DuniaFilm21Utils.absoluteUrl(api.mainUrl, anchor.attr("href")) ?: return@mapNotNull null
                if (!DuniaFilm21Utils.isContentUrl(epUrl) && !epUrl.contains("episode", true) && !epUrl.contains("/eps/", true)) return@mapNotNull null
                val rawName = DuniaFilm21Utils.cleanText(anchor.text()).ifBlank {
                    DuniaFilm21Utils.cleanTitle(anchor.attr("title"))
                }.ifBlank { epUrl.trimEnd('/').substringAfterLast('/').replace('-', ' ') }
                api.newEpisode(epUrl) {
                    name = rawName
                    season = DuniaFilm21Utils.seasonNumber(rawName)
                    episode = DuniaFilm21Utils.episodeNumber(rawName)
                }
            }
            .distinctBy { it.data }
